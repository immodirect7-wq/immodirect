...
export async function generateMetadata({ params }: ListingPageProps): Promise<Metadata> {
    ...
    try {
        ...
        const description = `${pType} à ${listing.neighborhood}, ${listing.city}. ${listing.price?.toLocaleString()} ${pType?.includes('meublé') ? 'FCFA/jour' : 'FCFA/mois'}. ${(listing.description || "").substring(0, 150)}...`;
        ...
        <>
            ...
            <div className="container mx-auto px-4 py-6 max-w-2xl">
                ...
                <div className="space-y-2 mb-6">
                    ...
                    <p className="text-primary font-bold text-xl">{listing.price.toLocaleString()} {listing.propertyType?.includes('meublé') ? 'FCFA / jour' : 'FCFA / mois'}</p>
                    <p className="text-gray-600">{listing.neighborhood}, {listing.city}</p>
                    {!listing.propertyType?.includes('meublé') && <p className="text-sm text-gray-500">Avance: {listing.advanceMonths} mois</p>}
                    <div className="mt-3">
                        <NavigateButton
